      ...
      try {
        ...
        if (statsResponse.success) {
          ...
          setCounts(prevCounts => ({
            ...prevCounts,
            ...statsResponse.data,
            sponsors: statsResponse.data.sponsors || 0,
            projects: statsResponse.data.projects || 0,
            campaigns: statsResponse.data.campaigns || 0,
            stories: statsResponse.data.stories || 0,
            blog: statsResponse.data.blog || 0,
            gallery: statsResponse.data.gallery || 0,
            team: statsResponse.data.team || 0,
            categories: statsResponse.data.categories || 0
          }));
        } else {
          console.error('Stats response:', statsResponse);
        ...
        if (activityResponse.success) {
          ...
          // Add recent blogs
          activityResponse.data.recent_blogs?.forEach((blog) => {
          ...
          // Add recent projects
          activityResponse.data.recent_projects?.forEach((project) => {
            transformedActivity.push({
              id: idCounter++,
              type: 'project',
              action: 'created',
          ...
          activityResponse.data.recent_campaigns?.forEach((campaign) => {
            transformedActivity.push({
              id: idCounter++,
              type: 'campaign',
              action: 'created',
  ...
  return (
    <AdminAuthGuard>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatsCard 
          title="Sponsors" 
          value={String(counts.sponsors || 0)} 
...
function StatsCard({ title, value, icon, link }) {
  return (
    <div onClick={() => router.visit(link)} className="cursor-pointer">
