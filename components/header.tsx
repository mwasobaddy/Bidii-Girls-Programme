          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-700 dark:text-gray-300 hover:text-[#e51083] dark:hover:text-[#e51083] font-medium transition-colors duration-300 relative group"
                onClick={handleNavClick}
              >
                {t(item.name.toLowerCase())}
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#e51083] transition-all duration-300 group-hover:w-full"></span>
              </Link>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="flex items-center space-x-2 bg-transparent">
                  ...
                  <nav className="flex flex-col space-y-4">
                    {navigation.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        onClick={handleNavClick}
                        className="text-lg font-medium text-gray-700 dark:text-gray-300 hover:text-[#e51083] dark:hover:text-[#e51083] transition-colors duration-300 py-2 border-b border-gray-200 dark:border-gray-700"
                      >
                        {t(item.name.toLowerCase())}
                      </Link>
                    ))}
